...
void matmul(float *A, float *B, float *C, int M, int N, int K,
  ...
  if (mpi_rank == 0) {
    ...
    MPI_Scatter(NULL, M * K / mpi_world_size, MPI_FLOAT, A, M * K / mpi_world_size, MPI_FLOAT, 0, MPI_COMM_WORLD);
  }
  ...
  for (int i=0; i<threads_per_process; i++) {
    matmul_thread(A, B, C, M/mpi_world_size, N, K, threads_per_process, i);
  }
  ...
  if (mpi_rank == 0) {
    ...
    MPI_Gather(C, M * N / mpi_world_size, MPI_FLOAT, NULL, M * N / mpi_world_size, MPI_FLOAT, 0, MPI_COMM_WORLD);
  }
