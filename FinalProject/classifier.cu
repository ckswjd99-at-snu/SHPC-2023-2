...
static __global__ void layernorm_1008_cuda(
  ...
  float *output_batch_offset = output + now_batch_idx * single_input_size;
  ...
  float4 temp_input, temp_gamma, temp_beta;
  for (int i = 0; i < LNORM_1008_INPT; i += 4) {
    temp_input = *reinterpret_cast<float4 *>(&input_batch_offset[threadIdx.x * LNORM_1008_INPT + i]);
    temp_gamma = *reinterpret_cast<float4 *>(&gamma[threadIdx.x * LNORM_1008_INPT + i]);
    temp_beta = *reinterpret_cast<float4 *>(&beta[threadIdx.x * LNORM_1008_INPT + i]);
    
    temp_input.x = (temp_input.x - mean1) / sqrtf(var + 1e-5) * temp_gamma.x + temp_beta.x;
    temp_input.y = (temp_input.y - mean1) / sqrtf(var + 1e-5) * temp_gamma.y + temp_beta.y;
    temp_input.z = (temp_input.z - mean1) / sqrtf(var + 1e-5) * temp_gamma.z + temp_beta.z;
    temp_input.w = (temp_input.w - mean1) / sqrtf(var + 1e-5) * temp_gamma.w + temp_beta.w;
...
static __global__ void layernorm_102_cuda(
  ...
  int input_offset = now_batch * single_input_size + threadIdx.x * LNORM_102_INPT;

  __shared__ float sum1[102 * LNORM_CHAN / LNORM_102_INPT];
  __shared__ float sum2[102 * LNORM_CHAN / LNORM_102_INPT];

  float psum1 = 0.0f, psum2 = 0.0f;
  for (int i = 0; i < LNORM_102_INPT; ++i) {
    psum1 += input[input_offset + i];
    psum2 += input[input_offset + i]
  ...
  if (threadIdx.x == 0) {
    for (int i = 1; i < 102 * LNORM_CHAN / LNORM_102_INPT; i++) {
      sum1[0] += sum1[i];
      sum2[0] += sum2[i];
    }
  }
  ...
  for (int i = 0; i < LNORM_102_INPT; ++i) {
    ...
    = (input[input_offset + i] - mean1)
    / sqrtf(var + 1e-5) * gamma[threadIdx.x * LNORM_102_INPT + i] + beta[threadIdx.x * LNORM_102_INPT + i];
  }
}
...
static __global__ void argmax_f4_cuda(float *input, float *output) {
  ...
  float *output_offset = output + now_batch;
...
private:
  ...
  // GPU
  cudaStream_t _gpu_stream_comp;
  cudaStream_t _gpu_stream_h2d;
  cudaStream_t _gpu_stream_d2h;
  int _gpu_idx;
  ...
  // Activations
  float *a_input_gpu[POP_BATCH_SIZE / COMPUTE_BATCH_SIZE], *a_conv1_gpu;
  ...
  float *a_linear2_gpu;
  float *a_linear3_gpu;
  float *a_argmax_gpu[POP_BATCH_SIZE / COMPUTE_BATCH_SIZE];
  float *a_argmax;
...
ComputeEngine::ComputeEngine(float *parameter_, int num_input_, int gpu_idx_) {
  ...
  CHECK_CUDA(cudaStreamCreate(&_gpu_stream_comp));
  CHECK_CUDA(cudaStreamCreate(&_gpu_stream_h2d));
  CHECK_CUDA(cudaStreamCreate(&_gpu_stream_d2h));

  // Initialize parameters
  CHECK_CUDA(cudaSetDevice(_gpu_idx));
  for (int b = 0; b < POP_BATCH_SIZE / COMPUTE_BATCH_SIZE; b++) {
    CHECK_CUDA(cudaMalloc(&a_input_gpu[b], COMPUTE_BATCH_SIZE * 70 * 1014 * sizeof(float)));
  }
  ...
  for (int b = 0; b < POP_BATCH_SIZE / COMPUTE_BATCH_SIZE; b++) {
    CHECK_CUDA(cudaMalloc(&a_argmax_gpu[b], COMPUTE_BATCH_SIZE * sizeof(float)));
  }
  ...
  CHECK_CUDA(cudaMemcpyAsync(
    ...
    cudaMemcpyHostToDevice, _gpu_stream_h2d
  ));
  CHECK_CUDA(cudaMemcpyAsync(
  ...
  CHECK_CUDA(cudaStreamSynchronize(_gpu_stream_h2d));
  
  // Initialize activations
  a_argmax = (float *)calloc(POP_BATCH_SIZE, sizeof(float));
...
void ComputeEngine::inference(float *popped_input) {
  ...
  cudaEvent_t h2d_mem[POP_BATCH_SIZE / COMPUTE_BATCH_SIZE];
  cudaEvent_t d2h_mem[POP_BATCH_SIZE / COMPUTE_BATCH_SIZE];


  for (int batch = 0; batch < POP_BATCH_SIZE; batch+=COMPUTE_BATCH_SIZE) {

    int now_batch_size = COMPUTE_BATCH_SIZE;

    CHECK_CUDA(cudaEventCreate(&h2d_mem[batch / COMPUTE_BATCH_SIZE]));
    CHECK_CUDA(cudaEventCreate(&d2h_mem[batch / COMPUTE_BATCH_SIZE]));

    // Conv block 1 : Conv1d + LayerNorm + ReLU + MaxPool1d
    {
      CHECK_CUDA(cudaMemcpyAsync(
        a_input_gpu[batch / COMPUTE_BATCH_SIZE], popped_input + batch * VOCAB_SIZE * MAX_LENGTH,
        now_batch_size * 70 * 1014 * sizeof(float),
        cudaMemcpyHostToDevice, _gpu_stream_h2d
      ));

      CHECK_CUDA(cudaEventRecord(h2d_mem[batch / COMPUTE_BATCH_SIZE], _gpu_stream_h2d));
      CHECK_CUDA(cudaStreamWaitEvent(_gpu_stream_comp, h2d_mem[batch / COMPUTE_BATCH_SIZE], 0));

      dim3 grid(CEIL_DIV(256, C1D_K7_BM), CEIL_DIV(1008, C1D_K7_BN));
      dim3 block(C1D_K7_BM * C1D_K7_BN);
      conv1d_k7_cuda<<<grid, block, 0, _gpu_stream_comp>>>(
        a_input_gpu[batch / COMPUTE_BATCH_SIZE], w_conv1_gpu, b_conv1_gpu, a_conv1_gpu,
        now_batch_size, 1008, 70, 256,
        0
      );

      layernorm_1008_cuda<<<now_batch_size, 1008*256/LNORM_1008_INPT, 0, _gpu_stream_comp>>>(
        a_conv1_gpu, gamma_conv1_gpu, beta_conv1_gpu, a_layernorm1_gpu,
        now_batch_size, 256, 1008
      );

      maxpool1d_k3_cuda<<<now_batch_size, 1008/3, 0, _gpu_stream_comp>>>(
    ...
    // Conv block 2 : Conv1d + ReLU + MaxPool1d
    {
      ...
      maxpool1d_k3_cuda<<<now_batch_size, 330/3, 0, _gpu_stream_comp>>>(
    ...
    // Conv block 3 : Conv1d + ReLU
    {
      ...
      conv1d_k3_cuda<<<grid, block, 0, _gpu_stream_comp>>>(
    ...
    // Conv block 6 : Conv1d + LayerNorm + ReLU + MaxPool1d
    {
      ...
      layernorm_102_cuda<<<now_batch_size, 102*256/LNORM_102_INPT, 0, _gpu_stream_comp>>>(
        a_conv6_gpu, gamma_conv6_gpu, beta_conv6_gpu, a_layernorm6_gpu,
        now_batch_size, 256, 102
      );

      maxpool1d_k3_cuda<<<now_batch_size, 102/3, 0, _gpu_stream_comp>>>(
    ...
    // FC block 1 : Linear + ReLU
    {
      ...
      linear_reg_cuda<<<grid, block, 0, _gpu_stream_comp>>>(
    ...
    // FC block 3 : Linear
    {
      ...
      linear_naive_cuda<<<grid, block, 0, _gpu_stream_comp>>>(
        a_linear2_gpu, w_fc3_gpu, b_fc3_gpu, a_linear3_gpu,
        now_batch_size, 1024, 4,
        0
      );

      argmax_f4_cuda<<<1, now_batch_size, 0, _gpu_stream_comp>>>(a_linear3_gpu, a_argmax_gpu[batch / COMPUTE_BATCH_SIZE]);
    }

    CHECK_CUDA(cudaEventRecord(d2h_mem[batch / COMPUTE_BATCH_SIZE], _gpu_stream_comp));
    CHECK_CUDA(cudaStreamWaitEvent(_gpu_stream_d2h, d2h_mem[batch / COMPUTE_BATCH_SIZE], 0));
  ...
  for (int batch = 0; batch < POP_BATCH_SIZE; batch+=COMPUTE_BATCH_SIZE) {
    int now_batch_size = COMPUTE_BATCH_SIZE;
    CHECK_CUDA(cudaMemcpyAsync(
      a_argmax + batch, a_argmax_gpu[batch / COMPUTE_BATCH_SIZE], now_batch_size * sizeof(float),
      cudaMemcpyDeviceToHost, _gpu_stream_d2h
    ));
  }
  
  CHECK_CUDA(cudaStreamSynchronize(_gpu_stream_d2h));

  for (int b = 0; b < POP_BATCH_SIZE; b++) {
    output_buf[num_input_processed + b] = a_argmax[b];
  }

  num_input_processed = num_input_processed + POP_BATCH_SIZE;
}
